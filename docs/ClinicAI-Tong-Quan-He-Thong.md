...
- **⚠️ Cần siết (BRIEF §3.3):** hiện **23 policy `FOR SELECT` cho `authenticated`** → gần như mọi nhân viên đọc được toàn bộ bệnh án/xét nghiệm/đơn thuốc/thai kỳ/thanh toán qua PostgREST. Hai lối siết theo bảng: **(A)** dời lệnh đọc ra sau FastAPI bằng `service_role` (= Phase 4); **(B)** policy lọc theo `release_state` cho CSKH. Đích: Lễ tân/Quản lý đọc `clinical_record` ra **0 dòng** (ROLE-02).
- **🔴 `care_episode` đang hỏng:** bật RLS, **0 policy** → PostgREST trả 0 dòng im lặng → màn `/episodes` rỗng. Cần thêm policy hoặc đọc qua service_role (BRIEF W1).
