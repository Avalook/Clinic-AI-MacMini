...
- **🟢 Tài khoản dùng chung đã bị xoá** (2026-08-02). Trước đó `CLINIC_SHARED_EMAIL` + màn `/enter` là một tài khoản Supabase duy nhất cho cả phòng khám; W3 đã làm nó đọc **0 dòng** (không có dòng `staff` ⇒ `current_clinic_ids()` rỗng), nên thứ còn lại chỉ là một mật khẩu ai cũng biết đứng trước trang đăng nhập thật. Nó cũng chặn đường bán nhiều phòng khám: một biến môi trường = một phòng khám. Nay cửa duy nhất là `/login` — một tài khoản một người.
- **🟢 Đã siết theo vai trò (ROLE-02, migration `20260730000013`).** Trong cùng một phòng khám, Lễ tân/Thu ngân/Quản lý nay đọc `clinical_record` + `clinical_form_response` ra **0 dòng**; chỉ 4 vai lâm sàng (`DOCTOR`/`ULTRASOUND_DOCTOR`/`TKYK`/`NURSE_ULTRASOUND`) đọc được — đúng danh sách `CLINICAL_WRITE_ROLES` phía backend, để hai tầng không trôi khỏi nhau. Chốt chặn cũ (`/home` đọc thẳng `clinical_record` bằng session người dùng) đã gỡ: nó gọi `GET /api/v1/visits/progress` và nhận **CỜ** ("đã đo sinh hiệu", "đã thu khâu nào"), không nhận nội dung bệnh án. `lab_result`/`prescription`/`payment`/`service_log` cố ý vẫn mở trong phạm vi phòng khám vì thu ngân và lễ tân cần để làm việc.
...
| 1 login/nhân viên | 🟡 code xong, chờ vận hành | Luồng đã đúng (`getCurrentStaff` suy từ `auth.uid()`), UI cấp tài khoản đã có ở `/settings/new-user`, cổng `/enter` dùng chung **đã xoá** (2026-08-02). Còn: cấp tài khoản cho từng nhân viên chưa link |
