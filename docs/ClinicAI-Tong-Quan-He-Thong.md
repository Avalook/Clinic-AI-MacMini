...
- **Tài khoản dùng chung** (`CLINIC_SHARED_EMAIL`, màn `/enter`) **đã bỏ hẳn 05/08/2026.** Nó vốn đọc 0 dòng ở mọi bảng vì không có dòng `staff`, chỉ còn tác dụng mở đường sang `/login`. Nay `/login` là cổng duy nhất, và proxy chặn luôn cả phiên không gắn nhân viên đang làm việc.
...
| 1 login/nhân viên | 🟢 xong | Luồng đã đúng (`getCurrentStaff` suy từ `auth.uid()`), UI cấp tài khoản ở `/settings/new-user`, cổng `/enter` dùng chung đã bỏ 05/08/2026. Còn lại là việc vận hành: cấp tài khoản cho từng nhân viên chưa link |
