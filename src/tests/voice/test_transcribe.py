...
def test_transcript_result_shape() -> None:
...
def test_construction_succeeds_without_deps(monkeypatch: pytest.MonkeyPatch) -> None:
...
@pytest.mark.asyncio
async def test_transcribe_raises_when_model_not_configured(
...
@pytest.mark.asyncio
async def test_transcribe_raises_when_faster_whisper_missing(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
