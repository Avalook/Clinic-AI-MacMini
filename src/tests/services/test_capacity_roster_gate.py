...
class _Conn:
    ...
    def __init__(self, *, roster_known: bool, on_duty: bool) -> None:
        self._duty = {"roster_known": roster_known, "on_duty": on_duty}
...
async def test_a_rostered_day_without_this_doctor_offers_nothing() -> None:
    ...
    conn = _Conn(roster_known=True, on_duty=False)
...
async def test_a_rostered_day_with_this_doctor_behaves_normally() -> None:
    conn = _Conn(roster_known=True, on_duty=True)
    out = await _quote(conn)
...
async def test_a_day_with_no_roster_yet_stays_open() -> None:
    ...
    """
    conn = _Conn(roster_known=False, on_duty=False)
    out = await _quote(conn)
