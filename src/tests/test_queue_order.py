...
def _e(appt, *, qn=None, checked_in=None, channel=None, b3=False, slot=SLOT):
    return QueueEntry(
        appointment_id=appt,
        doctor_id="d1",
        queue_number=qn,
...
# --------------------------- queue_rank --------------------------- #
def test_queue_rank_ut_numeric_other():
...
def test_booked_late_falls_to_tier1_by_arrival():
...
def test_walkin_tier1_by_arrival():
...
def test_pre_checkin_falls_back_to_ticket_order():
    e = _e("a", qn="7", channel=None, checked_in=None)
    assert call_rank(e)[:2] == (1, 7)


# --------------------------- full ordering --------------------------- #
def test_order_queue_priority_sequence():
...
# --------------------------- b3_ready_appt_ids --------------------------- #
def test_b3_ready_requires_all_resulted():
    labs = [
