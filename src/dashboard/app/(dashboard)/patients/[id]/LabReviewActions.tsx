    ...
    <div className="mt-2 flex flex-wrap items-center gap-2">
      {pending ? (
        <button
          ...
          className="rounded-md border border-[#db2777] px-2.5 py-1 text-xs font-semibold text-[#be185d] disabled:cursor-not-allowed disabled:opacity-40"
        >
        ...
        <button
          ...
          onClick={review}
          disabled={!hasResult || busy !== null}
          className="rounded-md bg-[#be185d] px-2.5 py-1 text-xs font-semibold text-white disabled:cursor-not-allowed disabled:opacity-40"
        >
          {busy === "review" ? "Đang hoàn tất…" : "Duyệt & hoàn tất"}
        </button>
      )}
      {message && (
        <span role="status" className="basis-full text-xs text-[#52525b]">
          {message}
        </span>
      )}
    </div>
