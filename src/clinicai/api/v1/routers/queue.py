...
async def get_queue(
    ...
    """Return today's CHECKED_IN patients as a flat list, ordered for calling."""
    day = date or date_cls.today()
    appt_rows = await pool.fetch(_APPT_SQL, day)
    appt_ids = [r["appointment_id"] for r in appt_rows]

    labs = await pool.fetch(_LAB_SQL, appt_ids) if appt_ids else []
