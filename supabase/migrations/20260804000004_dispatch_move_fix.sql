-- Chuyển PHÒNG không phải là đi thêm một BƯỚC.
--
-- Bản đầu của `move_visit_to_station` (20260804000003) luôn đóng bước cũ rồi mở
-- một work_item mới. Chạy thử trên dữ liệu thật thì nó vỡ ngay ở thao tác quan
-- trọng nhất của Trưởng ca — chuyển bệnh nhân từ SA1 sang SA2:
--
--     duplicate key value violates unique constraint "uq_work_item_visit_node_live"
--     Key (clinic_id, visit_id, node_code)=(…, DICHVU-SIEUAM) already exists
--
-- Ràng buộc đó đã có sẵn từ trước và nó nói một điều có chủ ý: MỘT LƯỢT KHÁM CHỈ
-- ĐI QUA MỖI BƯỚC MỘT LẦN. SA1 và SA2 là hai PHÒNG của cùng một bước "Thực hiện
-- siêu âm" — chuyển giữa chúng không phải làm siêu âm hai lần.
--
-- Bản này phân biệt hai việc:
--
--   cùng node  → ĐỔI PHÒNG: sửa room_id trên chính bước đang mở. Timeline vẫn
--                là một dòng "Siêu âm", đúng như những gì đã xảy ra với bệnh nhân.
--   khác node  → SANG BƯỚC KHÁC: đóng bước cũ, mở bước mới. Nếu bệnh nhân quay
--                lại một bước đã đi qua (đổi tuyến giữa chừng), MỞ LẠI chính
--                dòng đó thay vì chèn dòng thứ hai — tôn trọng ràng buộc sẵn có
--                thay vì gỡ nó ra cho tiện.
--
-- Cả hai nhánh vẫn ghi event_log, nên nhật ký điều phối không mất một lần chuyển
-- nào.
...
BEGIN
    ...
    IF p_node_code IS DISTINCT FROM v_old_node THEN
        ...
        ON CONFLICT (clinic_id, visit_id, node_code) WHERE status <> 'CANCELLED'
        DO UPDATE SET status = 'PENDING',
                      room_id = EXCLUDED.room_id,
                      started_at = now(),
                      finished_at = NULL,
                      updated_at = now(),
                      payload = public.work_item.payload
...
DO $verify$
BEGIN
    RAISE NOTICE 'move_visit_to_station: phân biệt đổi phòng và sang bước khác';
END
$verify$;
